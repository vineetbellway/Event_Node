...
exports.get_subscription_plan = async (req, res) => {
  var id = req.params.id;
  if (!id) {
    res.status(400).send({ status: false, message: "id missing" });
  } else {
    try {
      await SubscriptionPlan.aggregate([
        {
          $match: {
...
exports.delete_subscription_plan = async (req, res) => {
  var id = req.params.id;
  if (!id) {
    res.status(400).send({ status: false, message: "id missing" });
  } else {
...
exports.delete_subscription_plans = async (req, res) => {
  try {
...
exports.update_subscription_plan = (req, res, next) => {
  const id = req.params.id;
  if (!id) {
    res.status(400).send({ status: false, message: "id missing" });
  } else {
    try {
      SubscriptionPlan.findByIdAndUpdate(id, req.body, { new: true })
        .then((result) => {
          if (result) {
            res.status(201).send({
              status: true,
