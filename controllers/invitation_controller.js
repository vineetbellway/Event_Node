...
exports.get_invitations = async (req, res) => {
  ...
  try {
    var myAggregate = Invitation.aggregate([
      {
        $match: {
    ...
    await Invitation.aggregatePaginate(myAggregate, options)
      .then((result) => {
        if (result) {
          res.status(200).send({
            status: true,
...
exports.get_invitation = async (req, res) => {
...
exports.update_invitation = (req, res, next) => {
  const id = req.params.id;
  if (!id) {
    res.status(400).send({ status: false, message: "id missing" });
  } else {
    try {
      Invitation.findByIdAndUpdate(id, req.body, { new: true })
        .then((result) => {
          if (result) {
            res.status(201).send({
              status: true,
